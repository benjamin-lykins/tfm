...
var (
	...
	organizationCopyCmd = &cobra.Command{
		Use:     "organization",
		Short:   "Copy organization",
		Aliases: []string{"org"},
		Long:    "Copy organization from source to destination project",
...
func init() {
	...
	organizationCopyCmd.Flags().String("organization-name", "", "Specify the organization name to copy to destination")
	organizationCopyCmd.Flags().SetInterspersed(false)

	// Add commands
	CopyCmd.AddCommand(organizationCopyCmd)
}
...
func copyOrganization(c tfclient.ClientContexts, projMapCfg map[string]string) error {

	// Get Organization name
	organizationName := c.SourceOrganizationName

	createProject(c, organizationName)
